...
ft1<-rjd3tramoseats::tramo(ts, rjd3tramoseats::spec_tramo(name = "tr3") |> 
             rjd3toolkit::add_usrdefvar("r.group_1"),
           context = ws3$context) # ok

ft2<-rjd3tramoseats::tramo(ts, rjd3tramoseats::spec_tramo(name = "tr3") |> 
                                  rjd3toolkit::add_usrdefvar("r.group_1", coef = 0.003),
                                context = ws3$context) # ok
...
sp <- rjd3x13::spec_regarima(name = "rg3") |>
  rjd3toolkit::add_usrdefvar(id = "r.a")
rg<-rjd3x13::regarima(window(ts, start=1985, end=2013),sp,context = context) # ok

sp <- rjd3tramoseats::spec_tramo(name = "tr3") |>
  rjd3toolkit::add_usrdefvar(id = "r.a", coef = NULL)
